...
The current dashboard intentionally mirrors the handoff prototype at [nen/NEN Dashboard _standalone_.html](nen/NEN%20Dashboard%20_standalone_.html): dark command-center shell, summary strip, network node cards, desktop selected-node detail panel, and mobile bottom-sheet detail. State includes `token`, `theme` (`nen_design_theme`), `metrics`, `history`, `sites`, `devices`, selected site, local maintenance toggles, `siteViewMode`, Telegram alert settings, and transient one-time Telegram test token state. Polling defaults to 30 seconds via `VITE_POLL_INTERVAL_MS`.

The header Settings button opens a local settings modal with General, Notifications, Nodes, Runtime, and About sections. Telegram notification settings live in `localStorage` (`nen_telegram_settings`) but bot tokens must not be persisted in the browser. Telegram test delivery is server-backed through `GET /api/alerts/telegram/status` and `POST /api/alerts/telegram/test`; the server can use `TELEGRAM_BOT_TOKEN` from environment variables, or the UI can pass a one-time test token that is never saved. Full automatic alerts still need DB-backed channels/rules plus cooldown/recovery state.
...
- **server/.env**: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_DEFAULT_CHAT_ID`, and optional `TELEGRAM_DEFAULT_THREAD_ID` enable server-side Telegram test sends without entering a one-time token in the browser.
...
- **Merangin lithium mapping** — POP KOMINFO Merangin is now Huawei SMU02C/GSE200M lithium on SNMP `103.154.178.110:8091`, community `Anekanet`. It exposes Huawei ACB group `164.1.17` and lithium bank table `164.1.18` with active bank index `3669`, SOC, backup time, SOH, capacity, and 15 cell voltage/temperature readings. Keep it as `site_type='lithium'`; do not point it to `103.154.178.110:161` because that is the MikroTik RouterOS SNMP agent.
